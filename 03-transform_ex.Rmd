...
devtools::install("")
devtools::install_github("bss-osca/tfa/tfa-package", build=F)  # so have the latest version
library(tfa)
...
#write_csv(___)
# ___
write.csv(dat1,file1)
write.csv(dat2,file2)
...
```{r}
hotels %>%
  select(hotel:arrival_date_month) #vælger all kolonner mellem hotel og arrival
```
...
```{r}
hotels %>%
  select(starts_with("arrival")) #all kolonner der starter med arrival
```
...
hotels %>%
   select(ends_with("type")) #ends_with
```
...
hotels %>%
  select(adults, children, babies) %>%
  arrange(desc(babies), desc(children)) # så fx 2 børn så viser den de children der er tilsvarende størst til mindst
```
...
```{r}
hotels %>% 
  distinct(hotel, market_segment) %>% #kombineret
  arrange(hotel, market_segment)#først hotel og bahgefter segment
```
...
hotels %>%
  count(market_segment) # tæller de forskellige værdier i marketsegment
```

```{r}
# descending frequency order
hotels %>%
  count(market_segment, sort = TRUE) # sorter descending
```
...
```{r}
hotels %>%
  mutate(little_ones = children + babies) %>% #laver en ny kolonne
...
hotels %>%
  ...
  mutate(prop = n / sum(n))#antal hotel/total antal hotel 
```
...
hotels %>%
  summarise(mean_adr = mean(adr)) #gns ad adr kolonne
```
...
hotels %>%
  group_by(hotel) %>% 
  summarise(mean_adr = mean(adr))#ser mean fordelt på hotel 
```
...
hotels %>%
  group_by(hotel) %>%
  summarise(n = n())#n() er funktion for at tælle antal ligesom count
```
