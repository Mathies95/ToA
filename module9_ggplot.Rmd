...
ggplot(data = only_1962, 
       mapping = aes(x = life_expectancy, y = fertility, color = continent)) +
  geom_point()

filter(gapminder,year=="1962"|year=="2012")%>%
ggplot(mapping = aes(x = fertility, y = life_expectancy, color = continent)) +
  geom_point()+
  facet_grid(~year)

filter(gapminder,year %in%c(1962,1970,1980)& continent %in% c("Asia","Europe"))%>%
  ggplot(mapping = aes(fertility,life_expectancy,color=continent))+
  geom_point()+
  facet_wrap(~year)
?vars

filter(gapminder,continent=="Asia")%>%
  ggplot(mapping = aes(fertility,life_expectancy,color=year))+
  geom_point()
...
profit%>%
  ...
  ggplot(aes(x=reorder(`Product Line`,Total) ,Total))+
  geom_col()+
  labs(title = "My Title",
       x = "Line",
       y = "Profit"
...
profit%>%
  ...
  summarise(Total=sum(Profit))%>%
  ggplot(aes(x=reorder(`Customer ID`,Total),Total))+
  geom_col()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
profit%>%
  group_by(`Customer ID`)%>%
  summarise(ctr= n(),Total=sum(Profit))%>%
  ggplot(aes(x=reorder(`Customer ID`, Total),ctr,fill=Total))+
  geom_col()+
   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
dat%>%
  filter(type=="death" & country %in% c("China",
  "France",
  "Denmark",
  "US",
  "Italy"))%>%
  ggplot(aes(date,cumulative_cases,color=country))+
  geom_line()+
...
dat%>%
  ...
  ggplot(aes(date,tot_cases_pop,color=country))+
