...
str(flights)
dat %>%
  ggplot(aes(y=carrier_name, x=origin)) +
  geom_count() +
  labs(
    title = "Number of flights",
    y = "Carrier",
...
dat %>%
  count(month, carrier_name) %>%
  ggplot(mapping = aes(x = month, y = carrier_name, color = carrier_name)) +
  geom_line() +
  geom_point() +
  geom_dl(aes(label = "carrier"), method = list(dl.trans(x = x + .3), "last.bumpup")) +
...
dat %>% 
  group_by(carrier_name) %>% 
  summarise(ave_delay = mean(dep_delay, na.rm = TRUE)) %>%
  ggplot(aes(x=reorder(carrier_name, ave_delay),ave_delay))+
  geom_col()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(title = "flights",
       x = "carrier",
...
dat %>% 
  group_by(carrier_name) %>% 
  summarise(ave_delay = sd(dep_delay, na.rm = TRUE))
...
dat %>% 
  ...
  summarise(ave_delay = mean(dep_delay, na.rm = TRUE)) %>%
  ggplot(aes(x=reorder(origin_name, ave_delay),ave_delay,color=origin_name))+
  geom_col()
```
